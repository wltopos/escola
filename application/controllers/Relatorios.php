...
class Relatorios extends MY_Controller
{
    ...
    public function produtos()
    {
        ...
        $this->data['resultCategoria']  = $this->setdb_model->getTabelaQ('estoque_categorias');
        $this->data['resultSector']     = $this->setdb_model->getTabelaQ('estoque_sectors');
        $this->data['resultTipo']       = $this->setdb_model->getTabelaQ('estoque_tipo_produtos');
        $this->data['resultNota']       = $this->setdb_model->getTabelaQ('financeiro_notas');
        $this->data['resultLocation']   = $this->setdb_model->getTabelaQ('estoque_locations');
    ...
    public function produtosRapid()
    {
        ...
        $data['produtos'] = $this->setdb_model->getTabelaQ('estoque_produtos', $this->setquery_model->getFields('produtosRelatorio'), '',$this->setquery_model->getJoin('produtos'));
        $data['emitente'] = $this->Mapos_model->getEmitente();
    ...
    public function produtosCustom()
    {
        ...
        $tipoProduto    = $this->input->get('tipoProduto');
        $marca          = $this->input->get('marca');
        $sector         = $this->input->get('sector');
        $categoria      = $this->input->get('categoria');
        $dataCadastro   = $this->input->get('dataCadastro');
        $dataUpdate     = $this->input->get('dataUpdate');
        $notaFiscal     = $this->input->get('notaFiscal');
        $location       = $this->input->get('location');
        $addCampo       = $this->input->get('addCampo');
        

        $data['produtos'] = $this->Relatorios_model->produtosCustom($precoInicial, $precoFinal, $estoqueInicial, $estoqueFinal, $tipoProduto, $marca, $sector, $categoria, $dataCadastro, $dataUpdate, $notaFiscal, $location, $addCampo);
        ...
        if ($format == 'xls') {
            $vendasFormatadas = array_map(function ($item) {
                return [
                    ...
                    'totalDesconto' => $item['valor_pago'] ?: 0,
    ...
    public function receitasBrutasRapid()
    {
        ...
        $templatePath = realpath(FCPATH . "assets/uploads/db_estoque/relatorios/RELATORIO_MENSAL_DAS_RECEITAS_BRUTAS_MEI.docx");
        if (!$templatePath) {
        ...
        $tempFilePath = FCPATH . "assets" . DIRECTORY_SEPARATOR . "uploads" . DIRECTORY_SEPARATOR . "db_estoque".  DIRECTORY_SEPARATOR . "relatorios". DIRECTORY_SEPARATOR . "temp.docx";
        $generatedFilePath = FCPATH . "assets" . DIRECTORY_SEPARATOR . "uploads" . DIRECTORY_SEPARATOR . "db_estoque".  DIRECTORY_SEPARATOR . "relatorios". DIRECTORY_SEPARATOR . "RELATORIO_MENSAL_DAS_RECEITAS_BRUTAS_MEI_GERADO.$format"; 
