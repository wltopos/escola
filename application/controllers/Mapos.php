...
class Mapos extends MY_Controller
{
    ...
    {
        $this->data['db_empresa'] = $this->load->database($this->session->userdata('dbEmpresa'), true);
    ...
    public function emitente()
    {
        ...
        $this->data['dados'] = $this->setdb_model->getEmitente($this->session->userdata('id_administrativo_emitente'));
    ...
    public function do_upload()
    {
        ...
        $image_upload_folder = FCPATH . 'assets/uploads/'.$this->session->userdata('dbEmpresa').'/logoEmitente';
    ...
    public function do_upload_user()
    {
        ...
        $image_upload_folder = FCPATH . 'assets/uploads/'.$this->session->userdata('dbEmpresa').'/userImage';
    ...
    public function cadastrarEmitente()
    {
        ...
        $this->form_validation->set_rules('empresa', 'Razão Social', 'required|trim');
        $this->form_validation->set_rules('cnpj', 'CNPJ', 'required|trim');
        $this->form_validation->set_rules('ie', 'IE', 'required|trim');
        $this->form_validation->set_rules('cepEmitente', 'CEP', 'required|trim');
        $this->form_validation->set_rules('logradouroEmitente', 'Logradouro', 'required|trim');
        $this->form_validation->set_rules('numeroEmitente', 'Número', 'required|trim');
        $this->form_validation->set_rules('bairroEmitente', 'Bairro', 'required|trim');
        $this->form_validation->set_rules('cidadeEmitente', 'Cidade', 'required|trim');
        $this->form_validation->set_rules('ufEmitente', 'UF', 'required|trim');
        $this->form_validation->set_rules('telefoneEmitente', 'Telefone', 'required|trim');
        $this->form_validation->set_rules('emailEmitente', 'E-mail', 'required|trim');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $cep        = $this->input->post('cepEmitente');
            $logradouro = $this->input->post('logradouroEmitente');
            $numero     = $this->input->post('numeroEmitente');
            $bairro     = $this->input->post('bairroEmitente');
            $cidade     = $this->input->post('cidadeEmitente');
            $uf         = $this->input->post('ufEmitente');
            $telefone   = $this->input->post('telefoneEmitente');
            $email      = $this->input->post('emailEmitente');
            $image      = $this->do_upload();
            $logo       = base_url() . 'assets/uploads/'.$this->session->userdata('dbEmpresa')."/logoEmitente/$image";

            $retorno = $this->mapos_model->addEmitente($empresa, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo);
            if ($retorno) {
    ...
    public function editarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->mapos_model->editEmitente($id, $empresa, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email);
            if ($retorno) {
    ...
    public function editarLogo()
    {
        ...
        $this->load->helper('file');
        delete_files(FCPATH . 'assets/uploads/'.$this->session->userdata('dbEmpresa').'/logoEmitente');
    ...
    public function uploadUserImage()
    {
        ...
        if (is_file(FCPATH . 'assets/uploads/'.$this->session->userdata('dbEmpresa')."/userImage/$usuario->url_image_user")) {
            unlink(FCPATH . 'assets/uploads/'.$this->session->userdata('dbEmpresa')."/userImage/$usuario->url_image_user");
        }
    ...
    public function calendario()
    {
        ...
        $allLancamentos = $this->mapos_model->calendario(
            $start,
            $end,
            $status
        );
        $events = array_map(function ($lancamentos) {
            switch ($lancamentos->tipo) {
                case 'receita':
                    $cor = '#00cd00';
                    break;
                case 'despesa':
            ...
            return [
                'title' => "Lancamento: {$lancamentos->	id_financeiro_lancamento}, Cliente: {$lancamentos->cliente_fornecedor}",
                'start' => $lancamentos->data_vencimento,
                'end' => $lancamentos->data_vencimento,
                'color' => $cor,
                'extendedProps' => [
                    ...
                    'dataInicial' => '<b>Data Inicial:</b> ' . $lancamentos->data_pagamento,
                    'dataFinal' => '<b>Data Final:</b> ' . $lancamentos->data_vencimento,
                    'valorTotal' => '<b>Valor Total:</b> R$ ' . number_format($lancamentos->valor , 2, ',', '.'),
                    'valorDesconto' => '<b>Valor Desconto:</b> R$ ' . number_format($lancamentos->valor_ajuste, 2, ',', '.'),
                    'valorPago' => '<b>Valor Pago:</b> R$ ' . number_format($lancamentos->valor_pago, 2, ',', '.'),
                    'editar' => $this->financeiro_model->isEditable($lancamentos->id_financeiro_lancamento),
                ]
            ];
        }, $allLancamentos);
