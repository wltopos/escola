...
class Arquivos extends MY_Controller
{
    ...
    public function gerenciar()
    {
        ...
        if ($pesquisa == null && $de == null && $ate == null) {
            ...
            $this->data['configuration']['total_rows'] = $this->setdb_model->count('administrativo_documentos');

            $this->pagination->initialize($this->data['configuration']);

            $this->data['results'] = $this->setdb_model->getTabelaQ('administrativo_documentos', '*', '', $this->data['configuration']['per_page'], $this->uri->segment(3));
        } else {
            if ($de != null) {
                $de = explode('/', $de);
                $de = $de[2] . '-' . $de[1] . '-' . $de[0];
