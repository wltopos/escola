...
class Financeiro extends MY_Controller
{
    ...
    public function lancamentos()
    {
        ...
        if (!empty($cliente)) {
            if (empty($where)) {
                $where = "comercial_cliente_id LIKE '%$cliente%'";
            } else {
                $where .= " AND comercial_cliente_id LIKE '%$cliente%'";
            }
        }
        ...
        $this->data['configuration']['total_rows'] = $this->setdb_model->count('financeiro_lancamentos', $where);
        $this->data['configuration']['page_query_string'] = true;

        $this->pagination->initialize($this->data['configuration']);

        $this->data['results'] = $this->setdb_model->getTabelaQ('financeiro_lancamentos', '*', $where, '', '', '', $this->data['configuration']['per_page'], $this->input->get('per_page'));

        if (!empty($this->data['results'])) {
            $this->data['resultFuncionario'] = $this->setdb_model->check_credentials($this->data['results'][0]->administrativo_funcionario_id);
            $this->data['totals'] = $this->financeiro_model->getTotals($where);
            $this->data['estatisticas_financeiro'] = $this->financeiro_model->getEstatisticasFinanceiro();
        }
    ...
    public function editar()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $valor = $this->input->post('valor');
            $valor_com_desconto = $this->input->post('valor_desconto_editar');
            $valor_desconto = $valor  - $valor_com_desconto;
            $data = [
